...
class Dump{
	...
	public static function r(){
		...
		self::$_dumpData[] = ob_get_contents();
		ob_end_clean();
	}
	
	public static function getDumpData(){
...
class RHtmlFormatter extends RFormatter{
  ...
  public function endExp(){
  	$this->out .= "<{$this->def['base']} full-screen>FullScreen</{$this->def['base']}>";
  ...
  public function startRoot(){
  	//$timeFlg = time();
    $this->out .= '<!-- ref#' . ++static::$counter . ' --><div t='.static::$counter.'>' . static::getAssets() . '<div  t='.static::$counter.' class="ref">';
  }                    
...
class RTextFormatter extends RFormatter{
  ...
  public function startContain($type, $label = false){
    ...
    if($label)
      $this->out .= "\n" . str_repeat(' ', $this->indent + $this->levelPad[$this->level]) . "閳癸拷{$type[0]} ~ ";
  }    
